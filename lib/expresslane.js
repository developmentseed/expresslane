 ...
 */
var configure = function(settingsPath) {
    /**
     * Baseline config:
     */
    app.configure(function() {
        ...
        var settings = require(settingsPath + 'settings');
        ...
        try {
            ...
            var overrides = require(settingsPath + 'settings.env.' + env);
...
var start = function(settingsPath) {
    configure(settingsPath);
    // Let others modify app, require modules, then start server.
    if (app.set('settings')('modules')) {
        app.set('settings')('modules').forEach(function(module) {
            // Check for additional app configuration, require if found.
            try { require(module + '.expresslane.js').configure(app); } catch(error) {};
            try { require(module + '/lib/expresslane.' + module + '.js').configure(app); } catch(error) {};
        });
        // Lastly, require all the modules.
